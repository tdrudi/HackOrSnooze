...
function generateStoryMarkup(story, showDeleteBtn = false) {
  ...
  return $(`
      <li id="${story.storyId}">
        ...
        ${showFav ? favHTML(story, currentUser): ""}
...
function putUserStoriesOnPage(){
  ...
  if(currentUser.ownStories.length === 0)
    $ownStories.append("No stories added by user.");
  else{
...
function putFavoritesOnPage(){
  ...
  if(currentUser.favorites.length === 0)
    $favoritedStories.append("No favorited stories.");
  else{
...
$storiesList.on("click", ".star", toggleFavoriteStories);
